...
Blit_RGB888_RGB565Altivec(SDL_BlitInfo * info)
{
    ...
    while (height--) {
        ...
        vector unsigned char vCode_Overflow;
...
#define ONE_PIXEL_BLEND(condition, widthvar) \
        ...
        while (width) {
            ...
            vCode_Overflow = vec_ld(15, src);
            vsrc = vec_perm(vsrc, vCode_Overflow, valigner);
...
#define ONE_PIXEL_BLEND(condition, widthvar) \
        ...
        if (width > 0) {
            ...
            while (width) {
                ...
                vector unsigned int vCode_Overflow = vec_ld(15, srcp);
                /* load the source vec */
                vs = vec_perm(vs, vCode_Overflow, valigner);
...
ConvertAltivec32to32_noprefetch(SDL_BlitInfo * info)
{
    ...
    while (height--) {
        ...
        vector unsigned int vCode_Overflow;
        Uint32 bits;
        Uint8 r, g, b, a;
        ...
        while (width) {
            ...
            src += 4;
            width -= 4;
            vbits = vec_perm(vbits, vCode_Overflow, valigner);       /* src is ready. */
            vbits = vec_perm(vbits, vzero, vpermute);   /* swizzle it. */
            vec_st(vbits, 0, dst);      /* store it back out. */
            dst += 4;
            vbits = vCode_Overflow;
        }
